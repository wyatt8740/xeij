...
public class ADPCM {
  ...
  public static int pcmDivider;  //分周比の選択。0=1/1024,1=1/768,2=1/512,3=1/512
  ...
  public static void pcmInit () {
    ...
    for (int p = 0; p < 49; p++) {  //予測指標
      ...
      for (int n = 0; n < 16; n++) {  //4bitデータ
        ...
        p4[i] = Math.max (0, Math.min (48, p + ((n & 7) - 4 >> 3 | (n & 3) + 1 << 1)));  //次の予測指標。p+={-1,-1,-1,-1,2,4,6,8}[n&7]
        if (false) {
  ...
  public static void pcmFillBuffer (int endPointer) {
    if (Profiling.PFF_ON) {
      Profiling.pffStart[Profiling.PRF.pcmFillBuffer.ordinal ()] = System.nanoTime ();
    }
    ...
    if (Profiling.PFF_ON) {
      Profiling.pffTotal[Profiling.PRF.pcmFillBuffer.ordinal ()] += System.nanoTime () - Profiling.pffStart[Profiling.PRF.pcmFillBuffer.ordinal ()];
      Profiling.pffCount[Profiling.PRF.pcmFillBuffer.ordinal ()]++;
    }
  }  //pcmFillBuffer
  ...
  public static enum PIP {
    ...
    PIP_MONO_CONSTANT_16 {
      @Override public void write (int m) {
        if (Profiling.PFF_ON) {
          Profiling.pffStart[Profiling.PRF.pcmWriteBuffer.ordinal ()] = System.nanoTime ();
        }
        int i = pcmPointer;
        int p = pcmPanLeft;
        if (p > 0) {  //ON
        ...
        pcmPointer = i + 16;
        if (Profiling.PFF_ON) {
          Profiling.pffTotal[Profiling.PRF.pcmWriteBuffer.ordinal ()] += System.nanoTime () - Profiling.pffStart[Profiling.PRF.pcmWriteBuffer.ordinal ()];
          Profiling.pffCount[Profiling.PRF.pcmWriteBuffer.ordinal ()]++;
        }
      }
    },  //PIP_MONO_CONSTANT_16
