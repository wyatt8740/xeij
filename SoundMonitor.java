...
*/
  ...
  public static final int[] smnOPMBuffer = new int[SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES];
  public static final int[] smnPCMBuffer = new int[SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES];
  ...
  //スペクトラムアナライザ
  //  YM2151.OPM_SAMPLE_FREQ == 62500 && YM2151.OPM_BLOCK_SAMPLES == 2500 && SMN_WIDTH == 480 に固定
...
*/
  ...
  public static void smnInit () {
    ...
    //波形
    //smnOPMBuffer = new int[SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES];
    //smnPCMBuffer = new int[SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES];
    for (int scaleX = 0; scaleX <= SMN_WAVE_SCALE_X_MAX; scaleX++) {
      ...
      if (SoundSource.SND_CHANNELS == 1) {  //モノラル
        for (int x = 0; x < limit; x++) {
          index0[x] = x * YM2151.OPM_BLOCK_SAMPLES / limit;
        }
      } else {  //ステレオ
        int[] index1 = smnWaveIndex1[scaleX] = new int[limit];
        for (int x = 0; x < limit; x++) {
          index1[x] = (index0[x] = x * YM2151.OPM_BLOCK_SAMPLES / limit << 1) + 1;
        }
      }
    }
    smnWaveScaleX = 0;
  ...
  public static void smnMakeFrame () {
    ...
    //パネル
    smnPanel = XEiJ.setFixedSize (new JPanel () {
  ...
  public static void smnMakeImage () {
    ...
    for (int ch = 0; ch < 8; ch++) {
      YM2151.OPMChannel channel = YM2151.opmChannel[ch];
      //鍵盤
  ...
  public static void smnUpdate () {
    ...
    //波形
    System.arraycopy (YM2151.opmBuffer, 0, smnOPMBuffer, 0, SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES);
    System.arraycopy (ADPCM.pcmBuffer, 0, smnPCMBuffer, 0, SoundSource.SND_CHANNELS * YM2151.OPM_BLOCK_SAMPLES);
    smnWavePaint ();
    ...
    if (SoundSource.SND_CHANNELS == 1) {  //モノラル
      for (int i = SMN_SPECTRUM_N - 500, k = 0; i < SMN_SPECTRUM_N; i++, k += 5) {
        buffer5[i] = (double) (YM2151.opmBuffer[k    ] + ADPCM.pcmBuffer[k    ] +
                               YM2151.opmBuffer[k + 1] + ADPCM.pcmBuffer[k + 1] +
                               YM2151.opmBuffer[k + 2] + ADPCM.pcmBuffer[k + 2] +
                               YM2151.opmBuffer[k + 3] + ADPCM.pcmBuffer[k + 3] +
                               YM2151.opmBuffer[k + 4] + ADPCM.pcmBuffer[k + 4]) * (0.2 / 32768.0);
      }
    } else {  //ステレオ
      for (int i = SMN_SPECTRUM_N - 500, k = 0; i < SMN_SPECTRUM_N; i++, k += 10) {
        buffer5[i] = (double) (YM2151.opmBuffer[k    ] + YM2151.opmBuffer[k + 1] + ADPCM.pcmBuffer[k    ] + ADPCM.pcmBuffer[k + 1] +
                               YM2151.opmBuffer[k + 2] + YM2151.opmBuffer[k + 3] + ADPCM.pcmBuffer[k + 2] + ADPCM.pcmBuffer[k + 3] +
                               YM2151.opmBuffer[k + 4] + YM2151.opmBuffer[k + 5] + ADPCM.pcmBuffer[k + 4] + ADPCM.pcmBuffer[k + 5] +
                               YM2151.opmBuffer[k + 6] + YM2151.opmBuffer[k + 7] + ADPCM.pcmBuffer[k + 6] + ADPCM.pcmBuffer[k + 7] +
                               YM2151.opmBuffer[k + 8] + YM2151.opmBuffer[k + 9] + ADPCM.pcmBuffer[k + 8] + ADPCM.pcmBuffer[k + 9]) * (0.1 / 32768.0);
      }
    }
    //  区間4
    ...
    for (int ch = 0; ch < 8; ch++) {
      ...
      int m1 = channel.m1.slStage.ordinal () - YM2151.OPMStage.RELEASE.ordinal ();
      int m2 = channel.m2.slStage.ordinal () - YM2151.OPMStage.RELEASE.ordinal ();
      int c1 = channel.c1.slStage.ordinal () - YM2151.OPMStage.RELEASE.ordinal ();
      int c2 = channel.c2.slStage.ordinal () - YM2151.OPMStage.RELEASE.ordinal ();
      ...
      if (m1 > 0 && m2 > 0 && c1 > 0 && c2 > 0) {  //すべてSILENCE。キーオフ
        ...
        k1 = channel.chKeyIndex + ((3 << 6) + (64 - 38) - 768) >> 6;  //今回キーオンされているか最後にキーオンされたキー
      ...
      //音色
      {
        XEiJ.fmtHex2 (SMN_TONE_V0,  0, channel.chFLCON);  //FLCON
        XEiJ.fmtHex2 (SMN_TONE_V0,  3, channel.chSLOT);  //SLOT
        XEiJ.fmtHex2 (SMN_TONE_V0,  6, YM2151.opmWAVE256 >> 8);  //WAVE
        XEiJ.fmtHex2 (SMN_TONE_V0,  9, channel.chSYNC ? 1 : 0);  //SYNC
        XEiJ.fmtHex2 (SMN_TONE_V0, 12, 18 - Integer.numberOfTrailingZeros (YM2151.opmLFOPeriodMinor) << 4 | YM2151.opmLFOPeriodMajor - 16);  //SPEED
        XEiJ.fmtHex2 (SMN_TONE_V0, 15, YM2151.opmPMD);  //PMD
        XEiJ.fmtHex2 (SMN_TONE_V0, 18, YM2151.opmAMD);  //AMD
        XEiJ.fmtHex2 (SMN_TONE_V0, 21, channel.chPMS);  //PMS
        XEiJ.fmtHex2 (SMN_TONE_V0, 24, channel.chAMS);  //AMS
        XEiJ.fmtHex2 (SMN_TONE_V0, 27, channel.chPAN);  //PAN
        XEiJ.fmtHex2 (SMN_TONE_V1,  0, Math.max (0, channel.m1.slAttackRate - 32) >> 1);  //AR
        XEiJ.fmtHex2 (SMN_TONE_V2,  0, Math.max (0, channel.c1.slAttackRate - 32) >> 1);  //AR
        XEiJ.fmtHex2 (SMN_TONE_V3,  0, Math.max (0, channel.m2.slAttackRate - 32) >> 1);  //AR
        XEiJ.fmtHex2 (SMN_TONE_V4,  0, Math.max (0, channel.c2.slAttackRate - 32) >> 1);  //AR
        XEiJ.fmtHex2 (SMN_TONE_V1,  3, Math.max (0, channel.m1.slDecayRate - 32) >> 1);  //D1R
        XEiJ.fmtHex2 (SMN_TONE_V2,  3, Math.max (0, channel.c1.slDecayRate - 32) >> 1);  //D1R
        XEiJ.fmtHex2 (SMN_TONE_V3,  3, Math.max (0, channel.m2.slDecayRate - 32) >> 1);  //D1R
        XEiJ.fmtHex2 (SMN_TONE_V4,  3, Math.max (0, channel.c2.slDecayRate - 32) >> 1);  //D1R
        XEiJ.fmtHex2 (SMN_TONE_V1,  6, Math.max (0, channel.m1.slSustainRate - 32) >> 1);  //D2R
        XEiJ.fmtHex2 (SMN_TONE_V2,  6, Math.max (0, channel.c1.slSustainRate - 32) >> 1);  //D2R
        XEiJ.fmtHex2 (SMN_TONE_V3,  6, Math.max (0, channel.m2.slSustainRate - 32) >> 1);  //D2R
        XEiJ.fmtHex2 (SMN_TONE_V4,  6, Math.max (0, channel.c2.slSustainRate - 32) >> 1);  //D2R
        XEiJ.fmtHex2 (SMN_TONE_V1,  9, channel.m1.slReleaseRate - 34 >> 2);  //RR
        XEiJ.fmtHex2 (SMN_TONE_V2,  9, channel.c1.slReleaseRate - 34 >> 2);  //RR
        XEiJ.fmtHex2 (SMN_TONE_V3,  9, channel.m2.slReleaseRate - 34 >> 2);  //RR
        XEiJ.fmtHex2 (SMN_TONE_V4,  9, channel.c2.slReleaseRate - 34 >> 2);  //RR
        XEiJ.fmtHex2 (SMN_TONE_V1, 12, channel.m1.slDecayLevel >> YM2151.OPM_TL_BITS - 5 & 15);  //D1L
        XEiJ.fmtHex2 (SMN_TONE_V2, 12, channel.c1.slDecayLevel >> YM2151.OPM_TL_BITS - 5 & 15);  //D1L
        XEiJ.fmtHex2 (SMN_TONE_V3, 12, channel.m2.slDecayLevel >> YM2151.OPM_TL_BITS - 5 & 15);  //D1L
        XEiJ.fmtHex2 (SMN_TONE_V4, 12, channel.c2.slDecayLevel >> YM2151.OPM_TL_BITS - 5 & 15);  //D1L
        XEiJ.fmtHex2 (SMN_TONE_V1, 15, channel.m1.slTotalLevel >> YM2151.OPM_TL_BITS - 7);  //TL
        XEiJ.fmtHex2 (SMN_TONE_V2, 15, channel.c1.slTotalLevel >> YM2151.OPM_TL_BITS - 7);  //TL
        XEiJ.fmtHex2 (SMN_TONE_V3, 15, channel.m2.slTotalLevel >> YM2151.OPM_TL_BITS - 7);  //TL
        XEiJ.fmtHex2 (SMN_TONE_V4, 15, channel.c2.slTotalLevel >> YM2151.OPM_TL_BITS - 7);  //TL
        XEiJ.fmtHex2 (SMN_TONE_V1, 18, 5 - channel.m1.slKeyScale);  //KS
        XEiJ.fmtHex2 (SMN_TONE_V2, 18, 5 - channel.c1.slKeyScale);  //KS
        XEiJ.fmtHex2 (SMN_TONE_V3, 18, 5 - channel.m2.slKeyScale);  //KS
        XEiJ.fmtHex2 (SMN_TONE_V4, 18, 5 - channel.c2.slKeyScale);  //KS
        XEiJ.fmtHex2 (SMN_TONE_V1, 21, channel.m1.slMultiply >> 1);  //MUL
        XEiJ.fmtHex2 (SMN_TONE_V2, 21, channel.c1.slMultiply >> 1);  //MUL
        XEiJ.fmtHex2 (SMN_TONE_V3, 21, channel.m2.slMultiply >> 1);  //MUL
        XEiJ.fmtHex2 (SMN_TONE_V4, 21, channel.c2.slMultiply >> 1);  //MUL
        XEiJ.fmtHex2 (SMN_TONE_V1, 24, channel.m1.slDetune1Page >> 5);  //DT1
        XEiJ.fmtHex2 (SMN_TONE_V2, 24, channel.c1.slDetune1Page >> 5);  //DT1
        XEiJ.fmtHex2 (SMN_TONE_V3, 24, channel.m2.slDetune1Page >> 5);  //DT1
        XEiJ.fmtHex2 (SMN_TONE_V4, 24, channel.c2.slDetune1Page >> 5);  //DT1
        //perl -e "for$n(0,384,500,608){printf'%d ',$n*15>>7&3}"
        XEiJ.fmtHex2 (SMN_TONE_V1, 27, channel.m1.slDetune2Depth * 15 >> 7 & 3);  //DT2
        XEiJ.fmtHex2 (SMN_TONE_V2, 27, channel.c1.slDetune2Depth * 15 >> 7 & 3);  //DT2
        XEiJ.fmtHex2 (SMN_TONE_V3, 27, channel.m2.slDetune2Depth * 15 >> 7 & 3);  //DT2
        XEiJ.fmtHex2 (SMN_TONE_V4, 27, channel.c2.slDetune2Depth * 15 >> 7 & 3);  //DT2
        XEiJ.fmtHex2 (SMN_TONE_V1, 30, channel.m1.slAMSMask & 1);  //AMSEN
        XEiJ.fmtHex2 (SMN_TONE_V2, 30, channel.c1.slAMSMask & 1);  //AMSEN
        XEiJ.fmtHex2 (SMN_TONE_V3, 30, channel.m2.slAMSMask & 1);  //AMSEN
        XEiJ.fmtHex2 (SMN_TONE_V4, 30, channel.c2.slAMSMask & 1);  //AMSEN
        int x = SMN_TONE_X / 4 + SMN_TONE_BOX_COLS * (0b01_00_10_01_00_10_01_00 >> (ch << 1) & 3);
        int y = SMN_TONE_Y + SMN_TONE_BOX_HEIGHT * (0b10_10_01_01_01_00_00_00 >> (ch << 1) & 3);
