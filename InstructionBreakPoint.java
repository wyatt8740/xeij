...
public class InstructionBreakPoint {
  ...
  public static class InstructionBreakRecord {
    ...
    public int ibrThreshold;  //閾値。-1=インスタント
  ...
  //論理アドレス→命令ブレークポイント
  public static TreeMap<Integer,InstructionBreakRecord> ibpUserPointTable;  //ユーザモード用
  public static TreeMap<Integer,InstructionBreakRecord> ibpSuperPointTable;  //スーパーバイザモード用
  //物理ハッシュコード→その物理ハッシュコードを持つ命令ブレークポイントのリストの先頭
  public static InstructionBreakRecord[] ibpUserHashTable;  //ユーザモード用
  public static InstructionBreakRecord[] ibpSuperHashTable;  //スーパーバイザモード用
  ...
  public static void ibpInit () {
    if (IBP_ON) {
      ...
      ibpUserPointTable = new TreeMap<Integer,InstructionBreakRecord> ();
      ibpSuperPointTable = new TreeMap<Integer,InstructionBreakRecord> ();
      ibpUserHashTable = new InstructionBreakRecord[IBP_HASH_SIZE];
      ibpSuperHashTable = new InstructionBreakRecord[IBP_HASH_SIZE];
    }
  }  //ibpInit()
  ...
  public static void ibpReset () {
    if (IBP_ON) {
      for (InstructionBreakRecord r : ibpUserPointTable.values ()) {
        r.ibrValue = 0;
        r.ibrTarget = r.ibrThreshold < 0 ? 0 : r.ibrThreshold;
      }
      for (InstructionBreakRecord r : ibpSuperPointTable.values ()) {
        r.ibrValue = 0;
        r.ibrTarget = r.ibrThreshold < 0 ? 0 : r.ibrThreshold;
      }
    }
  ...
  public static void ibpInstant (int logicalAddress, int supervisor) {
    if (IBP_ON) {
      TreeMap<Integer,InstructionBreakRecord> pointTable = supervisor != 0 ? ibpSuperPointTable : ibpUserPointTable;
  ...
  //ibpPut (logicalAddress, supervisor, value, threshold, scriptElement)
  //  指定されたアドレスに命令ブレークポイントを設定する
  //  既に同じアドレスに命令ブレークポイントが設定されているときは閾値を変更する
  //  threshold  -1=インスタント
  public static void ibpPut (int logicalAddress, int supervisor, int value, int threshold,
    ...
    if (IBP_ON) {
      ...
      if (r == null) {  //指定されたアドレスに命令ブレークポイントが設定されていない
        ...
        {
          InstructionBreakRecord[] hashTable = supervisor != 0 ? ibpSuperHashTable : ibpUserHashTable;
