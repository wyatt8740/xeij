...
public class FDMedia extends HumanMedia {
  ...
  //ext = fdmPathToExt (path)
  //  パスから拡張子を取り出す
  //  拡張子がないときは""を返す。nullを返すことはない
  public static String fdmPathToExt (String path) {
    //拡張子を確認する
    int extIndex = path.lastIndexOf ('.');
    if (extIndex < 0) {  //拡張子がない
      return "";
    }
    String ext = path.substring (extIndex + 1).toUpperCase ();  //拡張子。大文字。'.'を含まない
    if (0 < extIndex && ext.equals ("GZ")) {
      int extIndex2 = path.lastIndexOf ('.', extIndex - 1);
      if (extIndex2 < 0) {  //拡張子がない
        return "";
      }
      ext = path.substring (extIndex2 + 1, extIndex).toUpperCase ();  //拡張子。大文字。'.'を含まない
    }
    return ext;
  }  //fdmPathToExt(String)

  //media = fdmExtToMedia (ext)
  //  拡張子に合うメディアを返す
  //  最初に見つかったメディアを返す
  public static FDMedia fdmExtToMedia (String ext) {
    for (FDMedia media : FDM_ARRAY) {
      for (String mediaExt : media.fdmExtensionArray) {
        if (ext.equals (mediaExt)) {  //拡張子が一致
          return media;
        }
      }
    }
    return null;
  }  //fdmExtToMedia(String)

  //media = fdmPathToMedia (path, bb)
  //  パスに合うメディアを返す
  //  バッファを指定したときはイメージを読み込む
  public static FDMedia fdmPathToMedia (String path, byte[] bb) {
    String ext = fdmPathToExt (path);  //拡張子
    if (ext.equals ("DIM")) {  //*.DIM
      if (bb == null) {
        bb = fdmCache;
      }
      //読み込む
      ...
      int length = XEiJ.ismRead (in, bb, 0, FDM_BUFFER_SIZE);
      XEiJ.ismClose (in);
      if (length < 256) {  //読み込めないまたは短すぎる
        return null;
      }
      //マジックを確認する
      if (!ByteArray.byaRstr (new StringBuilder (), bb, 171, 13).toString ().equals ("DIFC HEADER  ")) {  //マジックが一致しない
        return null;
      }
      //形式を確認する
      FDMedia media = null;
      ...
      //トラック保存状態をコピーする
      int tracksPerDisk = media.fdmTracksPerDisk;
      byte[] savedFlag = new byte[tracksPerDisk];
      System.arraycopy (bb, 1, savedFlag, 0, tracksPerDisk);
      //保存されているトラックの数を確認する
      int savedTracks = 0;
      for (int track = 0; track < tracksPerDisk; track++) {
        if (savedFlag[track] == 1) {  //保存されている
          savedTracks++;
        }
      }
      int bytesPerTrack = media.fdmBytesPerSector * media.fdmSectorsPerTrack;
      if (length != 256 + bytesPerTrack * savedTracks) {  //ファイルサイズが一致しない
        return null;
      }
      //ヘッダを潰して保存されているトラックを手前に詰める
      length -= 256;
      System.arraycopy (bb, 256, bb, 0, length);
      //保存されていないトラックを補間する
      for (int srcTrack = savedTracks - 1, dstTrack = tracksPerDisk - 1; 0 <= dstTrack; dstTrack--) {  //後ろから
        if (savedFlag[dstTrack] == 1) {  //保存されている
          System.arraycopy (bb, bytesPerTrack * srcTrack, bb, bytesPerTrack * dstTrack, bytesPerTrack);
          srcTrack--;
        } else {  //保存されていない
          Arrays.fill (bb, bytesPerTrack * dstTrack, bytesPerTrack * dstTrack + bytesPerTrack, (byte) 0xe5);
        }
      }
      return media;
    }
    //*.DIM以外
    //拡張子に合うメディアがあることを確認する
    if (fdmExtToMedia (ext) == null) {  //拡張子に合うメディアがない
      return null;
    }
    //ファイルサイズを確認する
    InputStream in = XEiJ.ismOpen (path);
    ...
    int length;
    if (bb != null) {
      length = XEiJ.ismRead (in, bb, 0, FDM_BUFFER_SIZE);
    } else {
      length = XEiJ.ismSkip (in, FDM_BUFFER_SIZE);
    }
    XEiJ.ismClose (in);
    //ファイルサイズと拡張子が一致するメディアを探す
    for (FDMedia media : FDM_ARRAY) {
      if (media.fdmBytesPerDisk == length) {  //ファイルサイズが一致
        for (String mediaExt : media.fdmExtensionArray) {
          if (ext.equals (mediaExt)) {  //ファイルサイズと拡張子が一致
            return media;
          }
        }
      }
  ...
  public FDMedia (int number,
    ...
    if (FDC.FDC_DEBUG_MEDIA && FDC.fdcDebugLogOn) {
      fdmPrintInfo ();
    }
  }  //new FDMedia(...)
