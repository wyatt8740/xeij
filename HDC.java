...
public class HDC {
  ...
  public static File hdcLastFile;  //最後にアクセスしたファイル＝次にファイルチューザーを開いたときに表示するディレクトリ。コマンドラインのみ

  //メニュー
  public static JMenu hdcMenu;

  //開くダイアログ
  public static JDialog hdcOpenDialog;  //ファイルチューザーダイアログ。コマンドラインのみ
  public static JFileChooser2 hdcOpenFileChooser;  //ファイルチューザー。コマンドラインのみ
  public static int hdcOpenUnit;  //開くユニットの番号。コマンドラインのみ
  public static boolean hdcOpenWriteProtect;  //true=ライトプロテクトモードで開く。コマンドラインのみ
  public static javax.swing.filechooser.FileFilter hdcOpenFileFilter;  //java.io.FileFilterと紛らわしい。*.HDFかどうかを調べるファイルフィルタ。ファイルチューザーとドロップターゲットで使う。コマンドラインのみ
  ...
  public static javax.swing.filechooser.FileFilter hdcFormatFileFilter;  //java.io.FileFilterと紛らわしい。*.HDFかどうかを調べるファイルフィルタ
  ...
  public static void hdcInit () {
    ...
    if (XEiJ.prgIsLocal) {  //ローカルのとき
      ...
      hdcLastFile = new File (".");  //カレントディレクトリ

      hdcOpenDialog = null;
      hdcOpenFileChooser = null;
      hdcOpenUnit = 0;
      hdcOpenWriteProtect = false;
      ...
      hdcOpenFileFilter = new javax.swing.filechooser.FileFilter () {  //java.io.FileFilterと紛らわしい
        @Override public boolean accept (File file) {
          ...
          if (true) {  //拡張子だけで判断する場合
            return (name.endsWith (".HDF") ||
                    name.endsWith (".HDF.GZ"));
          } else {  //ファイルサイズも確認する場合
            int length = 0;
            if (name.endsWith (".HDF")) {
              length = (int) file.length ();
            } else if (name.endsWith (".HDF.GZ")) {
              try (InputStream in = new GZIPInputStream (new FileInputStream (file))) {  //try-with-resourcesは1.7から
                length = XEiJ.ismSkip (in, 1024 * 1024 * 100);
              } catch (IOException ioe) {
              }
            }
            return HDMedia.hdmLengthToMedia (length) != null;
          }
        }
        @Override public String getDescription () {
          return Multilingual.mlnJapanese ? "SASI ハードディスクイメージ (*.HDF)" : "SASI Hard Disk Image (*.HDF)";
        }
      };
      hdcFormatFileFilter = new javax.swing.filechooser.FileFilter () {  //java.io.FileFilterと紛らわしい
        @Override public boolean accept (File file) {
          if (file.isDirectory ()) {
            return true;
          }
    ...
    for (int u = 0; u < 16; u++) {
      ...
      String path = Settings.sgsCurrentMap.get ("hd" + u);
    ...
    ActionListener listener = new ActionListener () {
      @Override public void actionPerformed (ActionEvent ae) {
        ...
        switch (command) {
        case "Create New SASI Hard Disk Images":  //SASIハードディスクイメージの新規作成
          hdcOpenFormatDialog ();
          break;
        }
      }
    ...
    //メニュー
    hdcMenu = XEiJ.createMenu ("SASI");  //横に長いとサブメニューを開きにくいので短くする
    XEiJ.addComponents (
      hdcMenu,
      XEiJ.createHorizontalBox (Multilingual.mlnText (XEiJ.createLabel ("SASI Hard Disk"), "ja", "SASI ハードディスク")),
    ...
    XEiJ.addComponents (
      hdcMenu,
      XEiJ.createHorizontalSeparator (),
      XEiJ.mdlSASIMenuItem,
      XEiJ.createHorizontalSeparator (),
      XEiJ.setEnabled (
        Multilingual.mlnText (XEiJ.createMenuItem ("Create New SASI Hard Disk Images", listener), "ja", "SASI ハードディスクイメージの新規作成"),
  ...
  //  開くダイアログを作る
  //  コマンドラインのみ
  public static void hdcMakeOpenDialog () {
    ActionListener listener = new ActionListener () {
      @Override public void actionPerformed (ActionEvent ae) {
        ...
        case "Reboot from it":  //ここから再起動
          {
            File[] list = hdcOpenFileChooser.getSelectedFiles2 ();
            if (list.length > 0) {
              hdcOpenFiles (list, true);
              hdcOpenDialog.setVisible (false);
            }
          }
          break;
        case "Open":  //開く
        ...
        case "Write-Protect":  //書き込み禁止
          hdcOpenWriteProtect = ((JCheckBox) ae.getSource ()).isSelected ();
          break;
        }
      }
    };
    hdcOpenFileChooser = new JFileChooser2 (hdcLastFile);
    hdcOpenFileChooser.setFileFilter (hdcOpenFileFilter);
    hdcOpenFileChooser.setMultiSelectionEnabled (true);  //複数選択可能
    hdcOpenFileChooser.setControlButtonsAreShown (false);  //デフォルトのボタンを消す
    hdcOpenFileChooser.addActionListener (listener);
    hdcOpenDialog = Multilingual.mlnTitle (
      XEiJ.createModalDialog (
        XEiJ.frmFrame,
        "Open SASI Hard Disk Images",
        XEiJ.createBorderPanel (
          ...
          XEiJ.createVerticalBox (
            ...
            XEiJ.createHorizontalBox (
              ...
              Multilingual.mlnText (XEiJ.createCheckBox (hdcOpenWriteProtect, "Write-Protect", listener), "ja", "書き込み禁止"),
              Box.createHorizontalGlue (),
  ...
  public static void hdcOpenFiles (File[] list, boolean reset) {
    for (int u = hdcOpenUnit, k = 0; k < list.length; ) {
      ...
      if (unit.insert (file.getPath ())) {  //挿入できた
        if (hdcOpenWriteProtect || !file.canWrite ()) {  //書き込みを禁止する
          unit.protect (false);  //開くときに書き込みを禁止した場合はイジェクトするまで書き込みを許可できない
        }
        hdcLastFile = file;  //最後にアクセスしたファイル
        u++;
      } else {
  ...
  public static void hdcMakeFormatDialog () {
    ...
    ActionListener listener = new ActionListener () {
      @Override public void actionPerformed (ActionEvent ae) {
        ...
        case "Start Formatting":  //フォーマットを開始する
          {
            File[] list = hdcFormatFileChooser.getSelectedFiles2 ();
            if (list.length > 0) {
    ...
    //ファイルチューザー
    hdcFormatFileChooser = new JFileChooser2 (hdcLastFile);
    hdcFormatFileChooser.setFileFilter (hdcFormatFileFilter);
        ...
        XEiJ.createBorderPanel (
          ...
          XEiJ.createVerticalBox (
            ...
            XEiJ.createHorizontalBox (
              Box.createHorizontalStrut (12),
              Box.createHorizontalGlue (),
              XEiJ.createRadioButtonMenuItem (mediaGroup, hdcFormatMedia == HDMedia.HDM_10MB,  "10MB", listener),
              XEiJ.createRadioButtonMenuItem (mediaGroup, hdcFormatMedia == HDMedia.HDM_20MB,  "20MB", listener),
              XEiJ.createRadioButtonMenuItem (mediaGroup, hdcFormatMedia == HDMedia.HDM_40MB,  "40MB", listener),
              Box.createHorizontalGlue (),
              Box.createHorizontalStrut (12)
              ),
            Box.createVerticalStrut (12),
            XEiJ.createHorizontalBox (
              Box.createHorizontalStrut (12),
              Box.createHorizontalGlue (),
              Multilingual.mlnText (XEiJ.createCheckBox (hdcFormatCopySystemFiles, "Copy System Files", listener), "ja", "システムファイルを転送する"),
              Box.createHorizontalGlue (),
              Box.createHorizontalStrut (12),
  ...
  public static void hdcOpenFormatDialog () {
    ...
    hdcFormatFileChooser.setSelectedFile (hdcLastFile);  //最後にアクセスしたファイルを設定する
...
*/
  ...
  public static class HDUnit extends AbstractUnit {
    ...
    public boolean hduFlush () {
      ...
      if (!XEiJ.ismSave (hduImage, 0, hduMedia.humDiskEndByte, abuPath, true)) {
        return false;
      }
      hduWritten = false;
      return true;
    ...
    @Override protected boolean open () {
      ...
      if (XEiJ.prgIsLocal) {  //ローカルのとき
        ...
        hdcOpenFileChooser.setSelectedFile (hdcLastFile);  //最後にアクセスしたファイルを設定する
        hdcOpenFileChooser.rescanCurrentDirectory ();  //挿入されているファイルが変わると選択できるファイルも変わるのでリストを作り直す
    ...
    @Override protected boolean load (String path) {
      if (false) {
        if (!XEiJ.ismLoad (hduImage, 0, (int) HDMedia.HDM_40MB.humDiskEndByte, path)) {  //読み込めない
          return false;
        }
      } else {
        InputStream in = XEiJ.ismOpen (path);
        if (in == null) {  //読み込めなかった
          return false;
        }
        int length = XEiJ.ismRead (in, hduImage, 0, HDMedia.HDM_MAX_BYTES_PER_DISK);  //実際に読み込んだ長さ
        XEiJ.ismClose (in);
        hduMedia = HDMedia.hdmLengthToMedia (length);  //メディアの種類
        if (hduMedia == null) {  //不明
          return false;
        }
      }
      hduWritten = false;
      if (XEiJ.prgIsLocal) {  //ローカルのとき
        hdcLastFile = new File (path);  //最後にアクセスしたファイルを更新する
      }
      ...
      if (false) {
        ...
        if (error) {
          XEiJ.prgMessage (Multilingual.mlnJapanese ? "40MB から 20MB へ変換できませんでした" : "Failed to convert from 40MB to 20MB");
        }
      }
      return true;
    }
