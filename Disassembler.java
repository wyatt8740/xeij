...
public class Disassembler {
  ...
  public static StringBuilder disScc (StringBuilder sb) {
    ...
    int ea = disOC & 63;
    if (XEiJ.EAM_DLT << ea < 0L) {
      return disEab (sb.append ('s').append (DIS_CC[cccc]).append (".b   "), ea);
    }
    int mmm = ea >> 3;
    int rrr = ea & 7;
    if (mmm == XEiJ.MMM_AR) {
      if (cccc != XEiJ.CCCC_T) {
        ...
        return XEiJ.fmtHex8 (disDR (disOp3 (sb, "db", cccc == XEiJ.CCCC_F ? "ra" : DIS_CC[cccc], ".w"), rrr).append (",$"), disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor));
      }
  ...
  public static StringBuilder disBra (StringBuilder sb) {
    ...
    if (offset == 0) {
      return XEiJ.fmtHex8 (sb.append ("bra.w   $"), disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor));
    } else if (offset == -1) {
      return XEiJ.fmtHex8 (sb.append ("bra.l   $"), disPC + MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor));
    } else {
      return XEiJ.fmtHex8 (sb.append ("bra.s   $"), disPC + offset);
    }
  ...
  public static StringBuilder disBsr (StringBuilder sb) {
    ...
    int offset = (byte) disOC;
    if (offset == 0) {
      return XEiJ.fmtHex8 (sb.append ("bsr.w   $"), disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor));
    } else if (offset == -1) {
      return XEiJ.fmtHex8 (sb.append ("bsr.l   $"), disPC + MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor));
    } else {
      return XEiJ.fmtHex8 (sb.append ("bsr.s   $"), disPC + offset);
    }
  ...
  public static StringBuilder disBcc (StringBuilder sb) {
    ...
    int offset = (byte) disOC;
    if (offset == 0) {
      return XEiJ.fmtHex8 (sb.append ('b').append (DIS_CC[cccc]).append (".w   $"), disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor));
    } else if (offset == -1) {
      return XEiJ.fmtHex8 (sb.append ('b').append (DIS_CC[cccc]).append (".l   $"), disPC + MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor));
    } else {
      return XEiJ.fmtHex8 (sb.append ('b').append (DIS_CC[cccc]).append (".s   $"), disPC + offset);
    }
  ...
  public static StringBuilder disPsave (StringBuilder sb) {
    int ea = disOC & 63;
    if (XEiJ.EAM_WTL << ea < 0L) {
      return disEaz (disOp1 (sb, "psave"), ea);
    }
    return disIllegal (sb);
  ...
  public static StringBuilder disPrestore (StringBuilder sb) {
    int ea = disOC & 63;
    if (XEiJ.EAM_RDL << ea < 0L) {
      return disEaz (disOp1 (sb, "prestore"), ea);
    }
    return disIllegal (sb);
  ...
  public static StringBuilder disFgen (StringBuilder sb) {
    ...
    case 0b010:  //Fop.* <ea>,FPn
      if (m == 7) {  //FMOVECR.X #ccc,FPn
        return disFPn (disHex2 (disOp1 (sb, "fmovecr.x").append ('#'), c).append (','), n);
      }
      //      LSXPWDBP
    ...
    case 0b100:  //FMOVEM.L <ea>,FPCR/FPSR/FPIAR
      {
        ...
        if ((m == 1 ? XEiJ.EAM_ALL : k == 1 ? XEiJ.EAM_DAT : XEiJ.EAM_ANY) << ea < 0L) {
          disOp1 (sb, k == 1 ? "fmove.l" : "fmovem.l");
          if (ea < XEiJ.EA_IM) {
            disEaz (sb, ea).append (',');
          } else {
            for (int i = 0; i < k; i++) {
    ...
    case 0b110:  //FMOVEM.X <ea>,<list>
      if (XEiJ.EAM_RDL << ea < 0L) {
        disEaz (disOp1 (sb, "fmovem.x"), ea).append (',');
  ...
  public static StringBuilder disFbccWord (StringBuilder sb) {
    ...
    if (c < DIS_FP_CC.length) {
      if (c == 0b000000) {  //FBF.W <label>
        ...
        return disHex8 (disOp1 (sb, "fbf.w"), base + offset);
      }
      if (c == 0b001111) {  //FBT.W <labsl> → FBRA.W <label>
        disStatus = DIS_ALWAYS_BRANCH;
        return disOW (disOp1 (sb, "fbra.w"));
      }
      //FBcc.W <label>
      disStatus = DIS_SOMETIMES_BRANCH;
  ...
  public static StringBuilder disFsave (StringBuilder sb) {
    int ea = disOC & 63;
    if (XEiJ.EAM_WTL << ea < 0L) {
      return disEaz (disOp1 (sb, "fsave"), ea);
    }
    return disIllegal (sb);
  ...
  public static StringBuilder disFrestore (StringBuilder sb) {
    int ea = disOC & 63;
    if (XEiJ.EAM_RDL << ea < 0L) {
      return disEaz (disOp1 (sb, "frestore"), ea);
    }
    return disIllegal (sb);
    ...
    if ((w & 0x0100) == 0) {  //ブリーフフォーマット
      ...
      if ((w & 0x0020) != 0) {  //ベースディスプレースメントあり
        if ((w & 0x0010) == 0) {  //ワードベースディスプレースメント
          disHex4 (sb, MC68060.mmuPeekWordZeroCode ((disPC += 2) - 2, disSupervisor)).append (".w");
        } else {  //ロングベースディスプレースメント
          disHex8 (sb, MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor)).append (".l");
        }
        sb.append (',');
      }
  ...
  public static StringBuilder disZW (StringBuilder sb) {
    return disHex4 (sb, MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor)).append (".w");
  }  //disZW(StringBuilder)
  public static StringBuilder disZL (StringBuilder sb) {
    return disHex8 (sb, MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor)).append (".l");
  }  //disZL(StringBuilder)
  public static StringBuilder disOW (StringBuilder sb) {
    return disHex8 (sb, disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor));
  }  //disOW(StringBuilder)
  public static StringBuilder disOL (StringBuilder sb) {
    return disHex8 (sb, disPC + MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor));
  }  //disOL(StringBuilder)
      ...
      if ((w & 0x0020) != 0) {  //ベースディスプレースメントあり
        if ((w & 0x0010) == 0) {  //ワードベースディスプレースメント
          if ((w & 0x0080) != 0) {  //ベースサプレスあり
            ...
            disHex8 (sb, disPC + MC68060.mmuPeekWordSignCode ((disPC += 2) - 2, disSupervisor)).append (".w");
          }
        } else {  //ロングベースディスプレースメント
          if ((w & 0x0080) != 0) {  //ベースサプレスあり
            disHex8 (sb, MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor)).append (".l");
          } else {  //ベースサプレスなし
            disHex8 (sb, disPC + MC68060.mmuPeekLongCode ((disPC += 4) - 4, disSupervisor)).append (".l");
          }
        }
        sb.append (',');
      }
  ...
  //  ニモニックと空白
  public static StringBuilder disOp1 (StringBuilder sb, String mn) {
    return sb.append (mn).append ("        ", Math.min (7, mn.length ()), 8);
  }  //disOp1(StringBuilder,String)
