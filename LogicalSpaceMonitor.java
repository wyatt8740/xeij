...
public class LogicalSpaceMonitor {
  ...
  public static void atwMake () {
    ...
    for (int m = 0; m < 2; m++) {
      //スクロールテキストエリア
      atwBoard[m] = XEiJ.setPreferredSize (
    ...
    atwFrame = Multilingual.mlnTitle (
      XEiJ.createRestorableSubFrame (
        ...
        XEiJ.createBorderPanel (
          XEiJ.createVerticalSplitPane (
            XEiJ.createVerticalBox (
              XEiJ.createHorizontalBox (
                Multilingual.mlnText (XEiJ.createLabel ("User Mode"), "ja", "ユーザモード"),
            ...
            XEiJ.createVerticalBox (
              XEiJ.createHorizontalBox (
                Multilingual.mlnText (XEiJ.createLabel ("Supervisor Mode"), "ja", "スーパーバイザモード"),
                Box.createHorizontalGlue ()
                ),
              atwBoard[1]
              )
        ...
        if (!(p0 == 1 && p1 == 1 ||
          ...
          if (p0 != 1) {
            ...
            if ((f0 & MC68060.MMU_DESCRIPTOR_WRITE_PROTECTED) != 0) {
              sb.append (Multilingual.mlnJapanese ? " [書き込み禁止]" : " [Write Protect]");
            }
            sb.append ('\n');
          }
          l0 = l1;
