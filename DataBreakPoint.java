...
public class DataBreakPoint {
  ...
  public static void dbpMakeFrame () {
    ...
    ActionListener listener = new ActionListener () {
      @Override public void actionPerformed (ActionEvent ae) {
        ...
        case "Add":  //追加
          new DataBreakRecord (0x00000000, DBP_BYTE, 0xff, 0, 0xff, false, 0, 0);
          break;
        }
      }
    };
    ...
    dbpListPanel = XEiJ.addComponents (
      new JPanel (dbpListGridLayout),
      Multilingual.mlnText (XEiJ.createLabel ("Enabled"), "ja", "有効"),
      Multilingual.mlnText (XEiJ.createLabel ("Address"), "ja", "アドレス"),
      Multilingual.mlnText (XEiJ.createLabel ("Size"), "ja", "サイズ"),
      Multilingual.mlnText (XEiJ.createLabel ("Mask"), "ja", "マスク"),
      Multilingual.mlnText (XEiJ.createLabel ("Lower"), "ja", "下限"),
      Multilingual.mlnText (XEiJ.createLabel ("Upper"), "ja", "上限"),
      Multilingual.mlnText (XEiJ.createLabel ("Count"), "ja", "回数"),
      Multilingual.mlnText (XEiJ.createLabel ("Threshold"), "ja", "閾値"),
      Multilingual.mlnText (XEiJ.createLabel ("Remove"), "ja", "削除")
      );
      ...
      XEiJ.createRestorableSubFrame (
        ...
        XEiJ.setPreferredSize (
          XEiJ.createBorderPanel (
            //CENTER
            XEiJ.createScrollPane (dbpListPanel),
            //NORTH
            null,
            //WEST
            null,
            //SOUTH
            XEiJ.createFlowPanel (
              FlowLayout.CENTER,
              Multilingual.mlnText (XEiJ.createButton ("Add", listener), "ja", "追加")
              )
            //EAST
            ),
          750, 100)
  ...
  public static void dbpBreak (int size, int address, int data) {
    if (size == DBP_BYTE) {  //バイトアクセス
      for (DataBreakRecord r : dbpList) {
        if (r.dbrEnabled) {  //ブレークポイントが有効
  ...
  public static class DataBreakRecord implements Comparable<DataBreakRecord> {
    ...
    //new DataBreakRecord (address, size, mask, lower, upper, enabled, count, threshold)
    //  コンストラクタ
    public DataBreakRecord (int address, int size, int mask, int lower, int upper, boolean enabled, int count, int threshold) {
      ...
      //有効
      dbrEnabledBox = XEiJ.createGlueBox (
        dbrEnabledCheckBox = Multilingual.mlnText (XEiJ.createCheckBox (dbrEnabled, "Enabled", listener), "ja", "有効")
        );

      //アドレス
      dbrAddressSpinner = XEiJ.createHex8Spinner (dbrAddress, 0xffffffff, false, new ChangeListener () {
      ...
      //マスク
      dbrMaskSpinner = XEiJ.createHex8Spinner (dbrMask, -1, false, new ChangeListener () {
        @Override public void stateChanged (ChangeEvent ce) {
          if (dbpLock == 0) {
      ...
      //下限
      dbrLowerSpinner = XEiJ.createHex8Spinner (dbrLower, -1, false, new ChangeListener () {
        @Override public void stateChanged (ChangeEvent ce) {
          if (dbpLock == 0) {
      ...
      //上限
      dbrUpperSpinner = XEiJ.createHex8Spinner (dbrUpper, -1, false, new ChangeListener () {
        @Override public void stateChanged (ChangeEvent ce) {
          if (dbpLock == 0) {
      ...
      //回数
      dbrCountSpinner = XEiJ.createNumberSpinner (
      ...
      //閾値
      dbrThresholdSpinner = XEiJ.createNumberSpinner (
      ...
      //削除
      dbrRemoveBox = XEiJ.createGlueBox (
        dbrRemoveButton = Multilingual.mlnText (XEiJ.createButton ("Remove", listener), "ja", "削除")
        );
    ...
    public void dbrSetEnabled (boolean enabled) {
      ...
      if (dbrEnabled) {  //有効
        ...
        for (DataBreakRecord r : dbpList) {
          if (r.dbrAddress >>> XEiJ.BUS_PAGE_BITS == p) {  //同じページに他のデータブレークポイントがある
            exists = true;
            break;
          }
        }
    ...
    public void dbrSetCount (int count) {
      if (dbrCount != count) {
        ...
        if (dbrCountSpinner != null) {
          dbpLock++;
          dbrCountSpinner.setValue (new Integer (count));
          dbpLock--;
        }
      }
    }  //dbr.dbrSetCount(int)
    ...
    public void dbrSetThreshold (int threshold) {
      if (dbrThreshold != threshold) {
        ...
        if (dbrThresholdSpinner != null) {
          dbpLock++;
          dbrThresholdSpinner.setValue (new Integer (threshold));
          dbpLock--;
        }
      }
