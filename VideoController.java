...
//      bit9-8  GrPr  グラフィックス画面の優先順位。0=手前,1=間,2=奥,3=単独
...
//       bit14  AHOn  1=グラフィックスカラーとテキストパレット0のカラーを混ぜる(A)
...
//    スプライト画面とテキスト画面とグラフィックス画面を3枚同時に重ね合わせることはしない
//    スプライト画面とテキスト画面のどちらかを代表する画面として残し、他方をグラフィックス画面と重ね合わせる前に除外する
...
//    グラフィックス画面と同じ要領でスプライトでバックグラウンドやテキストに穴を開けるといったことができる
//    グラフィックス画面のゼロプライオリティと異なるのはスプライト・バックグラウンドとテキストの間にグラフィックス画面が挟まっている
//    場合があることだが、グラフィックス画面よりも手前にあるスプライト・バックグラウンドまたはテキストによるゼロプライオリティで
//    グラフィックス画面よりも奥にあるスプライト・バックグラウンドまたはテキストにも穴が空くことに変わりはない
//    そのときグラフィックス画面のカラーが0ならは画面は黒になる
//
//  4bitページ
...
//
//  1番目
//    グラフィックス画面で最も手前に表示されるプレーンまたはそこに割り当てられているページまたはそれを構成するパレットコードの配列
//    G1On=1のときだけ表示される
//    512ドット16色のとき
...
//
//  2番目
//    グラフィックス画面で手前から2番目に表示されるプレーンまたはそこに割り当てられているページまたはそれを構成するパレットコードの配列
...
//
//  3番目
//    グラフィックス画面で手前から3番目に表示されるプレーンまたはそこに割り当てられているページまたはそれを構成するパレットコードの配列
//    G3On=1のときだけ表示される
//    512ドット16色のとき
//      4bitパレットコードのbit3-0はG3rdの4bitページから供給される
//
//  4番目
//    グラフィックス画面で手前から4番目に表示されるプレーンまたはそこに割り当てられているページまたはそれを構成するパレットコードの配列
//    G4On=1のときだけ表示される
//    512ドット16色のとき
...
//            なし
//
//  グラフィックス画面のプレーンの重ね合わせ
//    0でない1番目のパレット(なければ0)に対応するカラーがグラフィックス画面のカラーとして出力される
//    カラーが0ならばグラフィックス画面は透明になり、奥にあるスプライト画面またはテキスト画面が見える。奥に何もなければ画面は黒
//
//  グラフィックス画面のゼロプライオリティ
//    重ね合わせの仕組みから明らかなように、グラフィックス画面の中で手前にあるプレーンにパレットが0以外でカラーが0のピクセルを配置すると、
//    奥にあるパレットもカラーも0でない普通ならば透明にならないプレーンに穴が空いて、さらに奥にあるスプライト画面またはテキスト画面が見える
//
//  スプライト画面とテキスト画面とグラフィックス画面の優先順位
//    GrPr=0のとき
//      グラフィックス画面はスプライト画面とテキスト画面の手前
//                              TxPr
//                    0       1       2       3
//                 -------+
...
//    GrPr=3のとき
//      グラフィックス画面はスプライト画面とテキスト画面の手前で、スプライト画面とテキスト画面はOFF
//        グラフィックス画面とスプライト・テキスト画面の半透明を行うとテキストパレット0のカラーが反映されることから、
//        グラフィックス画面の奥にスプライト画面とテキスト画面があると解釈できる
...
//
//  スプライト画面とテキスト画面とグラフィックス画面の重ね合わせ
//    手前の画面のカラーが0以外(不透明)ならば手前の画面が表示される
//    手前の画面のカラーが0(透明)ならば奥の画面が表示される
//    例1
//      「テキスト＞グラフィックス」でテキスト画面のパレットが$0で不透明、グラフィックス画面も不透明のとき、
//       表示されていないスプライト画面の優先順位がグラフィックス画面よりも手前にあるか奥にあるかで表示が異なる
//       テキスト画面のパレットが$0で、表示されていないスプライト画面のパレットも$00なので、奥にある方が代表する画面になる
//      「テキスト＞スプライト＞グラフィックス」ならばスプライト画面のパレット$00(不透明)が表示され、
//      「テキスト＞グラフィックス＞スプライト」ならばグラフィックス画面(不透明)が表示される
//    例2
//      「バックグラウンド$30黄＞グラフィックス2赤＞テキスト0黒⇒黄」
//      「バックグラウンド$30黄＞グラフィックス2赤＞テキスト1灰⇒赤」
//      バックグラウンド(タイル張り最大15色)とグラフィックス(不透明であること以外に制約なし)のどちらを表示するか
//      テキストでドット毎に選択できる
//    例3
...
//            スプライトカラー
//          さもなくば(スプライトカラーが0のとき)
//            グラフィックスパレット0のカラー(0は黒)
//        さもなくば(スプライトパレットが0または(スプライトパレットの下位4bitが0かつテキストパレットが0でない)のとき)
//          グラフィックスパレット0のカラーが0でないとき
//            グラフィックスパレット0のカラー
//          さもなくば(グラフィックスパレット0のカラーが0のとき)
//            テキストカラー(0は黒)
//      1番目のパレットが2以上の偶数のとき
//        スプライトパレットの下位4bitが0でないまたは(スプライトパレットが0でないかつテキストパレットが0)のとき
//          スプライトカラーが0でないとき
...
//            テキストカラー(0は黒)
//          を混ぜたカラー(0は黒)
//    グラフィックス画面よりも奥にスプライト画面とテキスト画面が両方あるときはそれらを重ね合わせてから混ぜる
//    グラフィックス画面よりも手前にあるスプライト画面またはテキスト画面は通常通り出力される
//
//  半透明(XHPG)
//    概要
...
public class VideoController {
  ...
  //パレット
  //  ゼロ拡張
  public static final int VCN_PATTEST_BLOCKS = 10;  //パターンテスト用のパレットブロックの数。最後の2個はテキストエリア用、残りはスプライト用
  public static final int[] vcnPal16G8 = new int[256];  //グラフィックスパレット,変換前
  public static final int[] vcnPal16TS = new int[256 + 16 * VCN_PATTEST_BLOCKS];  //テキストスプライトパレット,変換前
  public static final int[] vcnPal32G8 = new int[256];  //グラフィックスパレット,変換後
  public static final int[] vcnPal32TS = new int[256 + 16 * VCN_PATTEST_BLOCKS];  //テキストスプライトパレット,変換後
  public static final int[] vcnPal8G16L = new int[256];  //65536色パレット,下位
  public static final int[] vcnPal8G16H = new int[256];  //65536色パレット,上位
  ...
  //(512ドット)4bitページアドレス
  //  4bitパレットは下位バイトにあるのでアドレスに1を加えておく
  //  OFFのプレーンをダミー4bitページに接続する
  //  すべてのプレーンがOFFのときはグラフィックス画面がOFFなので4bitページは参照されない
  public static int vcnHidden1st;  //1番目のプレーンの4bitページアドレス+1
  public static int vcnHidden2nd;  //2番目のプレーンの4bitページアドレス+1
  public static int vcnHidden3rd;  //3番目のプレーンの4bitページアドレス+1
  public static int vcnHidden4th;  //4番目のプレーンの4bitページアドレス+1
  public static int vcnVisible1st;  //1番目のプレーンの4bitページアドレス+1。OFFのときXEiJ.BUS_DUMMY_4BIT_PAGE+1
  public static int vcnVisible2nd;  //2番目のプレーンの4bitページアドレス+1。OFFのときXEiJ.BUS_DUMMY_4BIT_PAGE+1
  public static int vcnVisible3rd;  //3番目のプレーンの4bitページアドレス+1。OFFのときXEiJ.BUS_DUMMY_4BIT_PAGE+1
  public static int vcnVisible4th;  //4番目のプレーンの4bitページアドレス+1。OFFのときXEiJ.BUS_DUMMY_4BIT_PAGE+1
  ...
  public static void vcnInit () {
    ...
    //----------------------------------------------------------------------------------------
    //E  512ドット16色
    //             GrMM_SpTx_4         GrMM_SpTx_4
    for (int ii = 0b000_0000_0; ii <= 0b000_1111_1; ii++) {
      //                       GxOn                  GrMM_SpTx
      int i = ii << 13 - 0 & 0x2000 | ii << 2 - 1 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
      //----------------------------------------------------------------------------------------
      //GrPr==0
    ...
    //F  512ドット256色
    //             GrMM_SpTx_4         GrMM_SpTx_4
    for (int ii = 0b001_0000_0; ii <= 0b001_1111_1; ii++) {
      //                       GxOn                  GrMM_SpTx
      int i = ii << 13 - 0 & 0x2000 | ii << 2 - 1 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
    ...
    //G  512ドット65536色
    //             GrMM_SpTx_4         GrMM_SpTx_4
    for (int ii = 0b010_0000_0; ii <= 0b011_1111_1; ii++) {
      //                       GxOn                  GrMM_SpTx
      int i = ii << 13 - 0 & 0x2000 | ii << 2 - 1 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
    ...
    //----------------------------------------------------------------------------------------
    //H  1024ドット16色
    //             GrMM_SpTx_3210         GrMM_SpTx_3210
    for (int ii = 0b100_0000_0000; ii <= 0b100_1111_1111; ii++) {
      //                      3210                  GrMM_SpTx
      int i = ii << 9 - 0 & 0x1e00 | ii >> 4 - 2 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
      //----------------------------------------------------------------------------------------
      //GrPr==0
    ...
    //I  1024ドット256色
    //             GrMM_SpTx_3210         GrMM_SpTx_3210
    for (int ii = 0b101_0000_0000; ii <= 0b101_1111_1111; ii++) {
      //                      3210                  GrMM_SpTx
      int i = ii << 9 - 0 & 0x1e00 | ii >> 4 - 2 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
    ...
    //J  1024ドット65536色
    //             GrMM_SpTx_3210         GrMM_SpTx_3210
    for (int ii = 0b110_0000_0000; ii <= 0b111_1111_1111; ii++) {
      //                      3210                  GrMM_SpTx
      int i = ii << 9 - 0 & 0x1e00 | ii >> 4 - 2 & 0b111_111100;
      int s = i >> 4 & 3;
      int t = i >> 2 & 3;
  ...
  public static void vcnReset () {
    ...
    vcnHidden1st = 0x00c00000 + 1;
    vcnHidden2nd = 0x00c00000 + 1;
    vcnHidden3rd = 0x00c00000 + 1;
    vcnHidden4th = 0x00c00000 + 1;
    vcnVisible1st = XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
    vcnVisible2nd = XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
    vcnVisible3rd = XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
    vcnVisible4th = XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
  }  //vcnReset()
  ...
  public static void vcnUpdateMode () {
    ...
    if (vcnReg1Curr < 4) {  //512ドット
      ...
      vcnHidden1st = 0x00c00000 + 1 + ((vcnReg2Curr & 3     ) << 19    );
      vcnHidden2nd = 0x00c00000 + 1 + ((vcnReg2Curr & 3 << 2) << 19 - 2);
      vcnHidden3rd = 0x00c00000 + 1 + ((vcnReg2Curr & 3 << 4) << 19 - 4);
      vcnHidden4th = 0x00c00000 + 1 + ((vcnReg2Curr & 3 << 6) << 19 - 6);
      vcnVisible1st = on1st ? vcnHidden1st : XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
      vcnVisible2nd = on2nd ? vcnHidden2nd : XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
      vcnVisible3rd = on3rd ? vcnHidden3rd : XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
      vcnVisible4th = on4th ? vcnHidden4th : XEiJ.BUS_DUMMY_4BIT_PAGE + 1;
    }
    CRTC.crtAllStamp += 2;
  }  //vcnUpdateMode()
  ...
  //vcnUpdate ()
  //  スクリーンを更新する
  //  CRTC.crtDirtyY0>=0のときだけ呼び出すこと
  public static void vcnUpdate () {
    if (XEiJ.pnlZoomRatioOut == 1 << 16) {  //拡大なし
      XEiJ.pnlPanel.repaint (0L, XEiJ.pnlScreenX, XEiJ.pnlScreenY + CRTC.crtDirtyY0, XEiJ.pnlStretchWidth, CRTC.crtDirtyY1 - CRTC.crtDirtyY0 + 1);
    } else {  //拡大あり
      int y0 = (CRTC.crtDirtyY0 - 1) * XEiJ.pnlZoomRatioOut >> 16;
      int y1 = (CRTC.crtDirtyY1 + 2) * (XEiJ.pnlZoomRatioOut + 1) >> 16;
  ...
  public static void vcnReportUnimplemented (DrawingMode mode) {
    ...
    if (!vcnUnimplementedSet.contains (name)) {  //未報告
      XEiJ.prgMessage (new StringBuilder ().append (Multilingual.mlnJapanese ? "未実装 " : "Unimplemented ").append (name).toString ());
  ...
  public static StringBuilder vcnGetName (StringBuilder sb) {
    ...
    //グラフィックス画面
    String graphics = ((vcnReg1Curr & 0b100) != 0 ?  //1024ドット
    ...
    if (grpr == 0) {  //グラフィックス＞スプライトまたはテキスト＞スプライトまたはテキスト
      sb.append (graphics).append (spriteText1).append (spriteText2);
    } else if (grpr == 1) {  //スプライトまたはテキスト＞グラフィックス＞スプライトまたはテキスト
      sb.append (spriteText1).append (graphics).append (spriteText2);
    } else if (grpr == 2) {  //スプライトまたはテキスト＞スプライトまたはテキスト＞グラフィックス
      sb.append (spriteText1).append (spriteText2).append (graphics);
    } else {  //グラフィックス＞スプライトまたはテキスト(OFF)＞スプライトまたはテキスト(OFF)
      sb.append (graphics).append (spriteText1).append (spriteText2);
    }
    int i = sb.length ();
  ...
  //vcnMix2 (x, y)
  //  半透明合成
  //  x=G<<11|R<<6|B<<1|I  手前のグラフィックス画面の色
  //  y=g<<11|r<<6|b<<1|i  奥のテキスト・スプライト画面の色
  //  z=G+g>>1<<11|R+r>>1<<6|B+b>>1<<1|i  合成された色。グラフィックス画面のIは使用しない
